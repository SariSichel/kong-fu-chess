...
int motionArrivalMs(const realtime::Motion& motion) {
    return motion.started_at_ms + motion.duration_ms;
}
...
void resolveMoveArrival(model::Board& board, const realtime::Motion& motion,
                        ...
                        std::vector<realtime::MoveResolution>& resolutions) {
    ...
    if (!destination.isEmpty()) {
        ...
        if (isJumpActiveAt(activeMotions, motion.destination)) {
            const model::Piece& jumper = board.cell(motion.destination);
            realtime::JumpCaptureEvent event;
            event.timestamp_ms = motionArrivalMs(motion);
            event.jump_square = motion.destination;
            event.jumper_type = jumper.type();
            event.jumper_color = jumper.color();
            event.victim_type = motion.piece_type;
            event.victim_color = motion.piece_color;
            event.victim_from = motion.source;
            event.victim_to = motion.destination;

            board.removePieceAt(motion.source);

            realtime::MoveResolution resolution;
            resolution.kind = realtime::MoveResolution::Kind::JumpCapture;
            resolution.jump_capture = event;
    ...
    event.timestamp_ms = motionArrivalMs(motion);
    event.piece_type = motion.piece_type;
    event.piece_color = motion.piece_color;
    event.from = motion.source;
    event.to = motion.destination;

    if (!destination.isEmpty()) {
        event.captured_type = destination.type();
        event.captured_color = destination.color();
    }
    ...
    realtime::MoveResolution resolution;
    resolution.kind = realtime::MoveResolution::Kind::CompletedMove;
...
std::vector<MoveResolution> RealTimeArbiter::advanceTime(int ms, model::Board& board) {
    elapsed_ms_ += ms;
    decreaseAllCooldowns(board, ms);

    std::vector<MoveResolution> resolutions;
    ...
    if (!completingMoves.empty()) {
        ...
        for (const Motion& motion : completingMoves) {
            resolveMoveArrival(board, motion, active_motions_, arrivedThisTick, resolutions);
        }
